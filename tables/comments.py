...
def get(comment_id):
    if str(comment_id).isdigit():
        comment = db.GqlQuery(
            "SELECT * FROM Comments WHERE __key__ = KEY('Comments', :comment_id) LIMIT 1", comment_id=long(comment_id))
...
def get_comments(article_id=None, order="DESC"):
    ...
    if str(article_id).isdigit():
        comments = db.GqlQuery(
            "SELECT * FROM Comments WHERE article= :article_id ORDER BY submit_datetime %s " % order, article_id=long(article_id))
...
def add(article_id, username, comment):
    comment = Comments(article=long(article_id),
